...
const envSchema = z.object({
  ...
  FRONTEND_URL: z.string().url("FRONTEND_URL must be a valid URL").optional().default("http://localhost:8000"),

  // Database
  DATABASE_URL: z.string().default("file:./dev.db"),

  // Optional legacy Privy config
...
function getSafeConfig(parsed: z.infer<typeof envSchema>): Record<string, string> {
  return {
    ...
    PRIVY_APP_ID: parsed.PRIVY_APP_ID ? `${parsed.PRIVY_APP_ID.substring(0, 8)}...` : "(not set)",
