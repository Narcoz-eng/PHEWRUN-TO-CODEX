...
export function validateProductionEnvironment(): {
  ...
  if (!process.env.FRONTEND_URL) {
    warnings.push("FRONTEND_URL is not set (auth redirect allowlist may be incomplete)");
  }
  ...
  const hasAnySmtp = hasSmtpHost || hasSmtpUser || hasSmtpPass || hasSmtpFrom;
  const hasFullSmtp = hasSmtpHost && hasSmtpUser && hasSmtpPass && hasSmtpFrom;
  if (hasAnySmtp && !hasFullSmtp) {
    warnings.push("SMTP is partially configured. Forgot-password emails may fail.");
  }
